#!/usr/bin/env python3
"""Fine-tune LoRA adapter for template-content task (QLoRA)."""
...
def main() -> None:
    ...
    tpl = cfg["template"]
    train_path = (CONFIG_PATH.parent / tpl["train_file"]).resolve()
    val_path = (CONFIG_PATH.parent / tpl["val_file"]).resolve()
    out_dir = (CONFIG_PATH.parent / cfg["output_dir"] / "template-lora").resolve()
    ...
    sft_config = SFTConfig(
        ...
        num_train_epochs=tpl["epochs"],
        per_device_train_batch_size=tpl["batch_size"],
        learning_rate=tpl["learning_rate"],
        logging_steps=10,
        save_strategy="epoch",
        max_length=tpl["max_seq_length"],
    ...
    print(f"Saved template LoRA adapter to {out_dir}")
